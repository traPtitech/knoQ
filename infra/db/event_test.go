...
func Test_createEvent(t *testing.T) {
	...
	t.Run("create event with place", func(t *testing.T) {
		...
		p.Rooms[0].RoomID = uuid.Nil
		p.Rooms[0].Place = "instant room"
		event, err := createEvent(r.db.Debug(), p)
		require.NoError(err)

		e, err := getEvent(eventFullPreload(r.db), event.ID)
		require.NoError(err)
		assert.NotEqual(uuid.Nil, e.Rooms[0].Room.ID)
		assert.Equal(p.Rooms[0].Place, e.Rooms[0].Room.Place)
	})
}
